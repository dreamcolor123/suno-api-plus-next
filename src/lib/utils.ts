...
export const waitForRequests = (page: Page, signal: AbortSignal): Promise<void> => {
  return new Promise((resolve, reject) => {
    const urlPattern = /https?:\/\/(?:img[a-zA-Z0-9]*\.hcaptcha\.com|hcaptcha-imgs[^/]*\.suno\.com|hcaptcha-assets[^/]*\.suno\.com|hcaptcha-endpoint[^/]*\.suno\.com|newassets\.hcaptcha\.com|imgs\.hcaptcha\.com)/i;
...
export const corsHeaders = {
  ...
  'Access-Control-Allow-Headers': 'Content-Type, Authorization, x-api-key, api-key, x-suno-pool, x-suno-account-affinity, Idempotency-Key',
  'Access-Control-Expose-Headers': 'X-Suno-Account-Affinity, Retry-After',
}
